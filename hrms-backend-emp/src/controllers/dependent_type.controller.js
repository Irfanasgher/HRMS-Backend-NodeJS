...
exports.create = async (req, res) => {
  // Create comment
  const addressType = {
    name_dependent_type: req.body.name_dependent_type,

    is_enable: 1,
  };
  await db.dependent_type
    .create(addressType)
    .then((data) => {
      res.send(data);
    })
...
exports.findById = async (req, res) => {
  db.dependent_type
    .findOne({ where: { id_dependent_type: req.params.id, is_enable: 1 } })
    .then((data) => {
      res.send(data);
    })
    .catch((err) => {
